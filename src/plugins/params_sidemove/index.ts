...
export default class ParamsSidemove {
  ...
  _matchedElements(text: string, cursorPosition: CursorPosition, regexp: RegExp, matchingFunc: MatchingFunc): Array<MatchedElement> {
    return Array.from(text.matchAll(regexp)).map((regexMatchedElement: any): MatchedElement => {
      ...
      matchedElement.endOfMatchedText = matchedElement.startOfMatchedText + matchedElement.matchedText.length;
